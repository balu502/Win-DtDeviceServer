    ...
    for(uint i=0;i<bitError.size();i++){
      if(bitError.test(i))
        switch(i)
        {
          ...
          case MEASHIGHTUBE_ERROR: ret=CODEERR::MEASHT_ERROR; break;
          case SAVEALLVIDEO_ERROR: ret=CODEERR::SAVEVIDEODISKFULL; break;
    ...
    for(uint i=0;i<bitError.size();i++)
      if(bitError.test(i)) {
        switch(i)
        {
          ...
          case MEASHIGHTUBE_ERROR: devErrorsText.append(tr("Found error in meas high of tube procedure.")); break;
          case SAVEALLVIDEO_ERROR: devErrorsText.append(tr("Found error in save video procedure. May be disk full.")); break;
