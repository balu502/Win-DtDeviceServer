...
enum DevErrors
{
  ...
  RW_USBBULK_ERROR,         // 14
  CMD_USBBULK_ERROR,        // 15 error in process CAN comamnd (but don't CAN error) may be HW don't work property (SD cadr fex)
  MOTORDRVLED_ERROR,        // 16 motor driver of LED error
  GETFRAME_ERROR,           // 17 error in input data counts don't correspond of request for device type
  PRERUNFAULT_ERROR,        // 18 test on run is bad
  PRERUNSEMIFAULT_ERROR,    // 19 1 time test on run is bad but second is OK
  PRERUNMEMTEST_ERROR,      // 20 memory test in prerun error
  PRERUNANALYSE_ERROR,      // 21 analyse get picture data un prerun error
  ...
  MEASHIGHTUBE_ERROR,       // 25 measure high of tube error
  SAVEALLVIDEO_ERROR        // 26 save all video data after run error
};
